...
function isValidUser(username, password) {
    return username === 'Qusai22' && password === '223',
        username === 'jerome' && password === 'bÃ¤st'
}
...
app.post('/portfolio/new', (req, res) => {
    ...
    if (req.session.isLoggedIn && req.session.isAdmin) {
        ...
        db.run(
            ...
            [pname, pyear, pdesc, ptype, pimgUrl],
            (error) => {
                if (error) {
                    const model = {
                        dbError: true,
...
app.post('/portfolio/edit/:id', (req, res) => {
    ...
    db.run('UPDATE portfolio SET pid = ?, pname = ?, pyear = ?, pdesc = ?, ptype = ?, pimgURL, WHERE pid =?', [pname, pyear, pdesc, ptype, pimgUrl, portfolioID], 
    (error) => {
        if (error) {
...
app.get('/portfolio/delete/:id', (req, res) => {
    ...
    if (req.session.isLoggedIn && req.session.isAdmin == true) {
        db.run("DELETE FROM portfolio WHERE pid=?", [pid], (error, portfolio) => {
            if (error) {
                const model = {
                    dbError: true,
                    theError: error,
